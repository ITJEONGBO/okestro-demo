...
<ul>
    ...
    <li><a class="active" href="/storage/domains">스토리지</a></li>
        <li><a class="side" id="act" href="/storage/domains">도메인</a></li>
    ...
    <div class="dashboard-container">
        ...
        <div class="widget">
            ...
            <table width="1100px">
                ...
                <c:if test="${empty SDomainVOList}">
                    <tbody>
                    <tr>
                        <td colspan="8" align="center">스토리지 도메인이 없음</td>
                    </tr>
                    </tbody>
                </c:if>
                <c:forEach var="domainList" items="${SDomainVOList}" varStatus="status">
                    <tbody align="center">
                    <tr>
                        <td>${domainList.status == "active" ? "연결" : "연결 해제" }</td>
                        <td>${domainList.name}</td>
                        <td>${domainList.description}</td>
                        <td>${domainList.type}</td>
                        <td>${domainList.storageType}</td>
                        <td>${domainList.storageFormat}</td>
                        <td>${ (domainList.diskUsed + domainList.diskFree) / Math.pow(1024,3)} GB</td>
                        <td>${domainList.diskFree / Math.pow(1024,3)} GB</td>
                    </tr>
                    </tbody>
                </c:forEach>
            </table>
