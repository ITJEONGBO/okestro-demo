...
public class VmController {
	...
	private final ItVmOperationService vmOperation;
	private final ItVmNicService vmNic;

	@GetMapping
	@ApiOperation(value = "가상머신 목록", notes = "전체 가상머신 목록을 조회한다")
	@ResponseBody
	@ResponseStatus(HttpStatus.OK)
	public Res<List<VmVo>> vms() {
		log.info("--- 가상머신 목록");
...
//		return vmService.setVnic();
	...
	public Res<Boolean> addVm(@RequestBody VmCreateVo vm) {
		log.info("--- 가상머신 생성");
	...
	@ResponseBody
	@ResponseStatus(HttpStatus.CREATED)
	public Res<Boolean> editVm(@PathVariable String id,
	...
	@ResponseBody
	@ResponseStatus(HttpStatus.OK)
	public Res<Boolean> deleteVm(@PathVariable String id) {
		boolean type = false;
		log.info("--- 가상머신 삭제");
	...
	public VmVo vm(@PathVariable String id) {
		log.info("--- 가상머신 일반");
		return vmService.getVmDetail(id);
	}
	...
	@ApiImplicitParam(name = "id", value = "가상머신 아이디", dataTypeClass = String.class)
	@ResponseBody
	public Res<List<NicVo>> nic(@PathVariable String id) {
		log.info("----- vm nic 일반 불러오기: " + id);
		return Res.safely { vmNic.findAllFromVm(id); }
	}
