...
<ul>
    ...
    <li><a class="active" href="/networks">네트워크</a></li>
        ...
        <div class="widget">
            ...
            <table width="1100px">
                ...
                <c:if test="${empty networkList}">
                    <tbody>
                    <tr>
                        <td colspan="6" align="center">네트워크가 없음</td>
                    </tr>
                    </tbody>
                </c:if>
                <c:forEach var="networklist" items="${networkList}" varStatus="status">
                    <tbody align="center">
                    <tr>
                        <td>${networklist.name}</td>
                        <td>${networklist.description}</td>
                        <td>${networklist.usage}</td>
                        <td>${networklist.vlan}</td>
                        <td>${networklist.label}</td>
                        <td>${ networklist.mtu != 0 ? networklist.mtu : "기본값 (1500)" }</td>
                    </tr>
                    </tbody>
                </c:forEach>
            </table>
